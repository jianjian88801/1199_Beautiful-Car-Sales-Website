<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="材料分类" prop="cailiaofenlei">
          <el-input v-model="ruleForm.cailiaofenlei" 
              placeholder="材料分类" clearable  :readonly="ro.cailiaofenlei"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `cailiaofenlei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.cailiaofenleiCrossAddOrUpdateFlag = false;
                  this.parent.search();
